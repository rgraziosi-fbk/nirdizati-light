...
class DatasetConfs:
    ...
    def __init__(self, dataset_name, where_is_the_file='',data=False):
        if dataset_name in ["traffic_fines_%s" % formula for formula in range(1, 3)]:
            ...
            # features for classifier
            self.dynamic_cat_cols = {dataset: ["Resource"]}  # i.e. event attributes
            self.static_cat_cols = {dataset: ['Diagnose', 'DiagnosticArtAstrup', 'DiagnosticBlood', 'DiagnosticECG',
                                        'DiagnosticIC', 'DiagnosticLacticAcid', 'DiagnosticLiquor',
                                        'DiagnosticOther', 'DiagnosticSputum', 'DiagnosticUrinaryCulture',
                                        'DiagnosticUrinarySediment', 'DiagnosticXthorax', 'DisfuncOrg',
                                        ...
                                        'SIRSCritTemperature',
                                        'SIRSCriteria2OrMore'] } # i.e. case attributes that are known from the start
            self.dynamic_num_cols = {dataset: ["start:timestamp","Leucocytes","CRP","LacticAcid"]}
            self.static_num_cols = {dataset: []}
            ...
            self.static_cat_cols = {dataset: ["PClaims", "CType", "ClType"]}
            self.dynamic_num_cols = {dataset: []}
            self.static_num_cols = {dataset: ["Age", "ClaimValue"]}
            ...
            self.static_num_cols = {dataset: ['AMOUNT_REQ']}
        elif dataset_name in ["sepsis_cases_3_start", "sepsis_cases_1_start", 'sepsis_cases_1_start_update']:
            #### Sepsis Cases settings ####
            dataset = dataset_name
            if where_is_the_file != '':
                self.filename = {dataset: where_is_the_file}
            ...
            self.dynamic_num_cols = {dataset: ["start:timestamp","timesincemidnight",
                                         "timesincelastevent", "timesincecasestart", "event_nr", "open_cases","Leucocytes","CRP","LacticAcid","month", "weekday", "hour"]}
            self.static_num_cols = {dataset: ['Age']}
        elif dataset_name in ["sepsis_cases_2_start"]:
            #### Sepsis Cases settings ####
            dataset = dataset_name
            if where_is_the_file != '':
                self.filename = {dataset: where_is_the_file}
