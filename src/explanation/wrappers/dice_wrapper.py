...
def dice_explain(CONF, predictive_model,cf_df,encoder,df,query_instances, features_to_vary, method, optimization, heuristic,
                 support=0.99):
   ...
   # y_pred = predictive_model.model.predict(query_instances)
    ...
    try:
        ...
        if not os.path.exists(model_path+dataset+'.decl'):
            #discovery = model_discovery(CONF, encoder, df, dataset, features_names, d4py,
            #                                    model_path)
            full_discovery =  model_discovery(CONF, encoder, df, dataset, features_names, d4py, model_path,support)
    except OSError as error:
        ...
        for k in [5, 10, 15, 20]:
            ...
            if method == 'genetic_conformance':
                dice_result = dice_query_instance.generate_counterfactuals(x,encoder=encoder, desired_class='opposite',
                                                                           ...
                                                                           total_CFs=k,dataset=dataset,model_path=model_path,
                optimization=optimization,heuristic=heuristic)
            else:
                dice_result = dice_query_instance.generate_counterfactuals(x,encoder=encoder, desired_class='opposite',
                                                                           verbose=False,
                                                                           posthoc_sparsity_algorithm='linear',
                                                                           features_to_vary=features_to_vary,
                                                                           total_CFs=k,dataset=dataset)
...
def evaluate_cf_list(cf_list,query_instance,cont_feature_index,cat_feature_index,df,y_pred,nr_of_cfs,predictive_model,
                     query_instances,continuous_features,categorical_features,ratio_cont):
...
def plausibility(query_instance, predictive_model, cf_list,nr_of_cfs, query_instances, y_pred, continuous_features,
...
def decode_cfs(encoder,df,dice_result):
    decoded_df = encoder.decode(df)
    cf_list_decoded = np.array(decoded_df.iloc[explanations.cf_examples_list[0].final_cfs_df.index, :])
    return cf_list_decoded
...
def model_discovery(CONF,encoder,df,dataset,features_names,d4py,model_path,support):
    dataset_confs = DatasetConfs(dataset, where_is_the_file='')
    df = pd.DataFrame(df, columns=features_names)
    encoder.decode(df)
    df.insert(loc=0, column='Case ID', value=np.divmod(np.arange(len(df)), 1)[0] + 1)
