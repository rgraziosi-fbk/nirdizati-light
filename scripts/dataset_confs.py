class DatasetConfs:
    ...
    def __init__(self, dataset_name, where_is_the_file='',data=False):
        if dataset_name in ["traffic_fines_%s" % formula for formula in range(1, 3)]:
        ...
        elif dataset_name in ["sepsis_cases_%s" % i for i in range(1, 5)]:
            ...
            # features for classifier
            self.dynamic_cat_cols = {dataset: ["Activity", 'org:group']}  # i.e. event attributes
            ...
            self.dynamic_num_cols = {dataset: ['CRP', 'LacticAcid', 'Leucocytes', "hour", "weekday", "month", "timesincemidnight",
        ...
        elif dataset_name in ['bpic2012_O_ACCEPTED-COMPLETE','bpic2012_O_CANCELLED-COMPLETE','bpic2012_O_DECLINED-COMPLETE']:
            dataset = dataset_name
            if where_is_the_file != '':
                self.filename = {dataset: where_is_the_file}
            else:
