...
TARGET_COLUMN = ["waiting_time", "caseid"]
...
df['available_time'] = df['available_time'].astype(str).str[:19]
...
df['available_time'] = pd.to_datetime(df['available_time'], utc=True, format="%Y-%m-%d %H:%M:%S")
#invalid_dates = df[pd.to_datetime(df['start:timestamp'], errors='coerce').isna()]
#print(invalid_dates)
##### compute waiting_time
df['waiting_time'] = (df['start:timestamp'] - df['available_time']).dt.total_seconds()
...
'''for caseid in caseid_unique:
    ...
    for idx, row in enumerate(group_case.iterrows()):
        if idx == len(group_case):
            next_activity.append(ACT_2_NUMBER['PAD'])
        else:
            next_act = group_case.iloc[idx+1]['concept:name']
            next_activity.append(ACT_2_NUMBER['next_act'])'''

#df['next_act'] = df['next_act']
...
scaler = MinMaxScaler()
y_scaled = scaler.fit_transform(df["waiting_time"].values.reshape(-1, 1)).flatten()
y.loc[:, 'waiting_time'] = y_scaled
...
joblib.dump(rfr_mean, PATH_SAVE_MODEL + '/' + NAME_EXPERIMENT + '_predictive_model_mean_wait.joblib')
...
joblib.dump(rfr_std, PATH_SAVE_MODEL + '/' + NAME_EXPERIMENT + '_predictive_model_std_wait.joblib')
joblib.dump(scaler, PATH_SAVE_MODEL + '/' + NAME_EXPERIMENT + '_predictive_model_scaler_wait.pkl')
