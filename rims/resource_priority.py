...
class ResourcePriority(object):
    ...
    def __init__(self, env: simpy.Environment, name: str, capacity: int, calendar: dict, start: datetime):
        ...
        self.resource_simpy = simpy.PriorityResource(env, self.capacity)
        self.start = start
        self.queue = []
    ...
    def request_CF(self):
        self.queue.append(self.resource_simpy.queue)
        request = self.resource_simpy.request(0)
        return request
    ...
    def define_stop_weekend(self, timestamp):
        if min(self.calendar['days']) > timestamp.weekday():
            monday = min(self.calendar['days']) - timestamp.weekday()
        else:
